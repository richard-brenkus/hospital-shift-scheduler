...
public class ActivityKafkaProducerConfiguration {
    ...
    @Bean
    public ProducerFactory<String, ActivityKafkaMessage> activityProducerFactory(ActivityKafkaProperties properties, ObjectMapper objectMapper) {
        Map<String, Object> producerProperties = new HashMap<>();

        producerProperties.put(ProducerConfig.BOOTSTRAP_SERVERS_CONFIG, properties.bootstrapServers());
        producerProperties.put(ProducerConfig.CLIENT_ID_CONFIG, properties.clientId());
        producerProperties.put(ProducerConfig.ACKS_CONFIG, "all");
        producerProperties.put(ProducerConfig.ENABLE_IDEMPOTENCE_CONFIG, true);
        producerProperties.put(ProducerConfig.MAX_IN_FLIGHT_REQUESTS_PER_CONNECTION, 5);
        producerProperties.put(ProducerConfig.DELIVERY_TIMEOUT_MS_CONFIG, 30_000);
        producerProperties.put(ProducerConfig.REQUEST_TIMEOUT_MS_CONFIG, 10_000);

        JsonSerializer<ActivityKafkaMessage> valueSerializer = new JsonSerializer<>(objectMapper);
    ...
    @Bean
    public KafkaTemplate<String, ActivityKafkaMessage> activityKafkaTemplate(@Qualifier("activityProducerFactory") ProducerFactory<String, ActivityKafkaMessage> producerFactory) {
