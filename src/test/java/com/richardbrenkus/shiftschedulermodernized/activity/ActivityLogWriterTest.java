...
class ActivityLogWriterTest {
    ...
    void persist_shouldForwardAllFieldsToRepositoryInsert() {
        ...
        ActivityEvent event = new ActivityEvent(EVENT_ID, OCCURRED_AT, "alice", Role.ADMIN, ActivityType.USER_CREATED, "User", "42", "Created user", true, null, metadata);

        writer.persist(event);

        verify(activityLogRepository).insertIfAbsent(eq(EVENT_ID), eq("USER_CREATED"), eq("alice"), eq("ADMIN"), eq("User"), eq("42"), eq("Created user"), eq(true), eq(null), eq("POST"), eq("/admin/add"), eq("1.2.3.4"), eq(OCCURRED_AT));
    }

    @Test
    void persist_shouldSucceed_evenWhenInsertIfAbsentReportsDuplicate() {
        when(activityLogRepository.insertIfAbsent(any(UUID.class), anyString(), anyString(), anyString(), any(), any(), any(), anyBoolean(), any(), any(), any(), any(), any(Instant.class))).thenReturn(0); // duplicate — no row inserted
    ...
    void persist_shouldRejectNullEvent() {
        assertThatThrownBy(() -> writer.persist(null)).isInstanceOf(NullPointerException.class).hasMessageContaining("event");
    }
