...
class ScheduleGenerationEngineTest {
    ...
    void shouldAssignUserAndIncrementHitCounter_whenAllRulesPass() {
        ...
        CalculationInput input = calculationInput(
                List.of(user), List.of(1), List.of(1), List.of()
        );
    ...
    void shouldReturnZeroHits_whenMinimalGapCheckFails() {
        ...
        when(scheduleRuleService.isWithinTotalShiftLimit(
                anyInt(), any(UserCalculationData.class), any(CalculationCounters.class)
        )).thenReturn(true);
        when(scheduleRuleService.isWithinRequestedWeekdayLimit(
                any(UserCalculationData.class), anyInt(), any(CalculationCounters.class)
        )).thenReturn(true);
        when(scheduleRuleService.isWithinRequestedWeekendLimit(
                any(UserCalculationData.class), anyInt(), any(CalculationCounters.class)
        )).thenReturn(true);
        when(scheduleRuleService.respectsMinimalGap(
                any(LocalDate.class),
                anyInt(),
                any(UserCalculationData.class),
                any(CalculatedScheduleMonth.class),
                anyInt()
        )).thenReturn(false);
    ...
    void shouldSkipUserWithoutMatchingPreferenceInMonth() {
        ShiftPreferenceCalculationData preference =
                new ShiftPreferenceCalculationData(
                        1,
                        1,
                        5,
                        0,
                        false,
                        false,
                        Set.of(LocalDate.of(2026, 12, 1))
                );

        UserCalculationData user =
                userData(Set.of(), preference);
        ...
        CalculatedScheduleMonth month =
                emptyCalculatedMonth(AUGUST_2026);
    ...
    void shouldOnlyProcessForceFillShiftTypes_whenCallingForceFillPath() {
        ...
        CalculationInput input = calculationInput(
                List.of(user), List.of(1, 2), List.of(1, 2), List.of(2)
        );
        CalculatedScheduleMonth month = emptyCalculatedMonth(AUGUST_2026);
        allowAllDtoRules();
    ...
    void shouldSkipForceFillShiftTypesInRegularPath() {
        UserCalculationData user =
                userData(Set.of(), anyDatePreference(2));
    ...
    void shouldOnlyAssignOnce_whenSameShiftTypeIsVisitedMultipleTimes() {
        UserCalculationData user = userData(Set.of(), anyDatePreference(1));
        CalculationInput input = calculationInput(
                List.of(user), List.of(1), List.of(1, 1), List.of()
        );
        CalculatedScheduleMonth month = emptyCalculatedMonth(AUGUST_2026);
        allowAllDtoRules();
        ...
        long assignmentsOnDay = month.getDays().stream()
                .filter(day -> day.getDate().equals(AUGUST_2026.atDay(3)))
                .flatMap(day -> day.getAssignments().stream())
                .filter(assignment -> assignment.shiftType() == 1)
                .count();
    ...
    private UserCalculationData userData(
    ...
    private CalculatedScheduleMonth emptyCalculatedMonth(YearMonth month) {
        List<CalculatedScheduleDay> days = IntStream.rangeClosed(
                        1,
                        month.lengthOfMonth()
                )
    ...
    private boolean hasAssignmentOnDay(
            CalculatedScheduleMonth month,
            int dayOfMonth,
            int shiftType
    ) {
        return month.getDays().stream()
                .filter(day -> day.getDate().equals(
                        AUGUST_2026.atDay(dayOfMonth)
                ))
                .flatMap(day -> day.getAssignments().stream())
