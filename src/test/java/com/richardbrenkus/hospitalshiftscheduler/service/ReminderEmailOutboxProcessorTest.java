...
class ReminderEmailOutboxProcessorTest {
    ...
    void processPendingReminderJobs_shouldMarkSentAndPublishSuccess_whenDispatchSucceeds() {
        ...
        when(completionService.markSent(eq(1L), eq("claim-token-1"), eq(NOW))).thenReturn(true);

        processor.processPendingReminderJobs();

        verify(emailReminderService).sendShiftRequestReminderEmail("alice@example.test", "Alice", LocalDate.of(2026, 8, 20), "idem-1");
        verify(completionService).markSent(eq(1L), eq("claim-token-1"), eq(NOW));
        verify(activityPublisher).publishSuccess(eq(ActivityType.REMINDER_EMAIL_SENT), eq("ReminderEmailOutbox"), eq("1"), any(String.class));
    }
    ...
    void processPendingReminderJobs_shouldScheduleRetry_onTransientFailure() {
        ...
        var job = claimedJob(1L, 1);
        when(claimService.claim(eq(1L), any(String.class), eq(NOW))).thenReturn(Optional.of(job));
        doThrow(new TransientEmailDeliveryException("smtp temporary")).when(emailReminderService).sendShiftRequestReminderEmail(any(), any(), any(), any());
        when(completionService.markTransientFailure(eq(1L), eq("claim-token-1"), eq(NOW), any(String.class))).thenReturn(ReminderEmailOutboxCompletionService.FailureCompletionResult.RETRY_SCHEDULED);

        processor.processPendingReminderJobs();

        verify(completionService).markTransientFailure(eq(1L), eq("claim-token-1"), eq(NOW), any(String.class));
        verify(completionService, never()).markSent(any(), any(), any());
        verify(activityPublisher).publishFailure(eq(ActivityType.REMINDER_EMAIL_FAILED), eq("ReminderEmailOutbox"), eq("1"), any(String.class), any(String.class), any());
    }
    ...
    void processPendingReminderJobs_shouldMoveToDead_onPermanentFailure() {
        ...
        var job = claimedJob(1L, 1);
        when(claimService.claim(eq(1L), any(String.class), eq(NOW))).thenReturn(Optional.of(job));
        doThrow(new PermanentEmailDeliveryException("invalid recipient")).when(emailReminderService).sendShiftRequestReminderEmail(any(), any(), any(), any());
        when(completionService.markPermanentFailure(eq(1L), eq("claim-token-1"), eq(NOW), any(String.class))).thenReturn(ReminderEmailOutboxCompletionService.FailureCompletionResult.DEAD);

        processor.processPendingReminderJobs();

        verify(completionService).markPermanentFailure(eq(1L), eq("claim-token-1"), eq(NOW), any(String.class));
        // DEAD → published as failure with dead-suffixed description
        ArgumentCaptor<String> description = ArgumentCaptor.forClass(String.class);
        verify(activityPublisher).publishFailure(eq(ActivityType.REMINDER_EMAIL_FAILED), eq("ReminderEmailOutbox"), eq("1"), description.capture(), any(String.class), any());
    ...
    void processPendingReminderJobs_shouldNotDispatch_whenClaimReturnsEmpty() {
        ...
        when(repository.findDispatchableIds(anyList(), eq(NOW), any(Pageable.class))).thenReturn(List.of(1L));
        when(claimService.claim(eq(1L), any(String.class), eq(NOW))).thenReturn(Optional.empty());
    ...
    void processPendingReminderJobs_shouldContinueLoop_whenOneJobFailsUnexpectedly() {
        when(repository.findDispatchableIds(anyList(), eq(NOW), any(Pageable.class))).thenReturn(List.of(1L, 2L));
        when(claimService.claim(eq(1L), any(String.class), eq(NOW))).thenThrow(new RuntimeException("claim exploded"));
        var okJob = claimedJob(2L, 1);
        when(claimService.claim(eq(2L), any(String.class), eq(NOW))).thenReturn(Optional.of(okJob));
        when(completionService.markSent(eq(2L), eq("claim-token-2"), eq(NOW))).thenReturn(true);

        processor.processPendingReminderJobs();

        // Second candidate must still be processed even if the first blew up.
        verify(emailReminderService).sendShiftRequestReminderEmail(any(), any(), any(), eq("idem-2"));
    }
    ...
    private static ReminderEmailOutboxClaimService.ClaimedReminderEmailJob claimedJob(long id, int attempt) {
        return new ReminderEmailOutboxClaimService.ClaimedReminderEmailJob(id, "claim-token-" + id, 99L + id, "alice@example.test", "Alice", LocalDate.of(2026, 8, 20), "idem-" + id, attempt);
    }
}
